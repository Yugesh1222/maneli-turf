...
(function () {
  ...
  function startScanner() {
    ...
    const scanConfig = { fps: 10, qrbox: { width: 260, height: 260 }, aspectRatio: 1.0 };
    const onSuccess = (decodedText) => onScanSuccess(decodedText.trim());

    html5QrCode
      .start(highRes, scanConfig, onSuccess)
      .then(() => { scanning = true; })
      .catch((err) => {
        console.warn("High-res camera request failed, falling back:", err);
        // Some phones reject strict width/height min constraints —
        // retry with just facingMode so scanning still works, even if
        // the picture ends up softer on that device.
        html5QrCode
          .start({ facingMode: "environment" }, scanConfig, onSuccess)
          .then(() => { scanning = true; })
          .catch((err2) => {
            console.error(err2);
